...
function BoosterTool({ toolKey }: { toolKey: string }) {
  ...
  const nothingSelected = !nick.trim() && !bio.trim() && !avatarB64 && !bannerB64;

  const handleBoost = async () => {
    if (!tokens.length || !guildId.trim() || nothingSelected) return;
    setRunning(true);
    setError("");
    setResults([]);
    setSummary(null);

    const res = await boostTokens(toolKey, tokens, guildId.trim(), {
      nick: nick.trim() || null,
      bio:  bio.trim()  || null,
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">Apply nick, bio, avatar & banner to multiple Discord tokens in a server</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-5">
            ...
            <div className="card-dark rounded-2xl p-5">
              ...
              <input
                ...
                disabled={running}
              />
              <p className="text-xs text-gray-600 mt-1.5">Right-click a server icon → Copy Server ID (dev mode must be on)</p>
            </div>
            ...
            <div className="card-dark rounded-2xl p-5">
              ...
              <textarea
                ...
                disabled={running}
              />
              <p className="text-xs text-gray-600 mt-1">{tokenCount} token{tokenCount !== 1 ? "s" : ""} · max 20 per run</p>
            </div>
            ...
            <div className="card-dark rounded-2xl p-5 space-y-4">
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">Nickname in server</label>
                <input
                  value={nick}
            ...
            <div className="card-dark rounded-2xl p-5">
              ...
              <div className="flex items-center gap-3">
                <input
                  type="range" min={200} max={3000} step={100} value={delayMs}
            ...
            <div className="card-dark rounded-2xl p-4" style={{ border: "1px solid rgba(59,130,246,0.3)", background: "rgba(59,130,246,0.04)" }}>
              <div className="flex items-start gap-2">
                ...
                <p className="text-gray-400 text-xs leading-relaxed">
                  Avatar/Banner require the account to have <span className="text-purple-300 font-semibold">Nitro</span>. Nick is applied per-server. Bio is global. Leave a field empty to skip it.
                </p>
              </div>
            </div>
            ...
            >
              {running
                ? <><Loader2 size={16} className="animate-spin" /> Boosting Profiles...</>
                : <><Zap size={16} /> Start Boosting</>
              }
            </button>
          </div>
                  ...
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs mt-0.5 ${r.success ? "text-green-300" : "text-red-300"}`}>
                          {r.message}
                        </p>
                      </div>
                    </div>
