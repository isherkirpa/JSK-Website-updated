...
export const handler = async (event) => {
  ...
  try {
    ...
    if (method === "GET" && path === "/keys") {
      ...
      if (adminPass !== stored) return json({ error: "Unauthorized" }, 401);
      const result = await client.query(
        "SELECT id, tool_id as \"toolId\", key, created_at as \"createdAt\", expires_at as \"expiresAt\", is_active as \"isActive\" FROM jsk_api_keys ORDER BY created_at DESC"
      );
    ...
    if (method === "POST" && path === "/keys") {
      const stored = await getAdminPassword(client);
      if (adminPass !== stored) return json({ error: "Unauthorized" }, 401);
      const { toolId, expiryHours } = body;
      const validTools = ["mass-dm", "token-checker", "onliner", "server-joiner", "booster"];
      ...
      await client.query(
        "INSERT INTO jsk_api_keys (tool_id, key, created_at, expires_at, is_active) VALUES ($1, $2, $3, $4, true)",
        [toolId, key, createdAt, expiresAt]
      );
      const row = await client.query(
        "SELECT id, tool_id as \"toolId\", key, created_at as \"createdAt\", expires_at as \"expiresAt\", is_active as \"isActive\" FROM jsk_api_keys WHERE key = $1",
        [key]
      );
    ...
    if (method === "DELETE" && path.startsWith("/keys/")) {
      ...
      await client.query("UPDATE jsk_api_keys SET is_active = false WHERE id = $1", [keyId]);
      return json({ success: true });
    }
    ...
    if (method === "POST" && path === "/bot/start") {
      ...
      const result = await botServerRequest("/api/token/start", "POST", { token, settings: settings ?? {} });
      return json(result);
    }
    ...
    if (method === "POST" && path === "/bot/boost") {
      const { key, tokens, guildId, nick, bio, avatarBase64, bannerBase64, delayMs } = body;
      if (!key || !tokens || !guildId) return json({ error: "key, tokens, and guildId required" }, 400);
      const keyResult = await validateKeyInternal(client, "booster", key);
      if (!keyResult.valid) return json({ error: "Invalid or expired key", reason: keyResult.reason }, 401);
      const result = await botServerRequest("/api/boost", "POST", {
        tokens, guildId, nick, bio, avatarBase64, bannerBase64, delayMs: delayMs ?? 500,
      });
      return json(result);
    }
