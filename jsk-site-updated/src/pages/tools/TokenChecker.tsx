...
import {
  ...
  Loader2, Play, Trash2, Copy, Check
} from "lucide-react";
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">Verify Discord tokens — valid, invalid, or expired</p>
            </div>
          </div>
        </div>
      </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-2 max-h-80">
                {results.map((r, i) => (
                  ...
                  >
                    ...
                    <p className="font-mono text-xs text-gray-500 truncate mb-1">{r.token.slice(0, 40)}...</p>
                    {r.status === "valid" && (
                      <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 mt-2">
                        <span className="text-xs text-gray-400">User: <span className="text-white">{r.username}</span></span>
                        <span className="text-xs text-gray-400">ID: <span className="text-purple-300">{r.id}</span></span>
                        <span className="text-xs text-gray-400">Email: <span className="text-gray-300">{r.email}</span></span>
                        <span className="text-xs text-gray-400">Phone: <span className="text-gray-300">{r.phone}</span></span>
