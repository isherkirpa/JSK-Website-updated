...
export type ToolId = "mass-dm" | "token-checker" | "onliner" | "server-joiner" | "booster" | "admin";
...
export type ValidateResult = { valid: boolean; expiresAt?: number | null; createdAt?: number; reason?: string; label?: string; };
...
export async function getAllEntries(adminPass: string): Promise<Record<ToolId, KeyEntry[]>> {
  ...
  const result: Record<ToolId, KeyEntry[]> = { "mass-dm": [], "token-checker": [], onliner: [], "server-joiner": [], booster: [], admin: [] };
  for (const entry of data.keys ?? []) {
    const id = entry.toolId as ToolId;
    if (result[id]) result[id].push(entry);
  }
  return result;
}
export function isKeyExpired(entry: KeyEntry): boolean {
  return !entry.isActive || (entry.expiresAt !== null && Date.now() > entry.expiresAt);
}
export async function createKey(toolId: ToolId, expiryHours: number, adminPass: string, label: string): Promise<KeyEntry | null> {
  const res = await fetch(`${NETLIFY_FN}/keys`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-admin-password": adminPass },
    body: JSON.stringify({ toolId, expiryHours, label }),
  });
  return await res.json();
}
...
export async function deleteExpiredKeys(adminPass: string): Promise<{ success: boolean; deleted: number }> {
  const res = await fetch(`${NETLIFY_FN}/admin/cleanup-expired`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-admin-password": adminPass },
  });
  return await res.json();
...
export async function getMyTokens(key: string): Promise<{ ok: boolean; tokens?: LiveTokenInfo[]; error?: string }> {
  try {
    const res = await fetch(`${NETLIFY_FN}/bot/my-tokens`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ key }) });
    return await res.json();
  } catch (e) { return { ok: false, error: String(e) }; }
}
...
export async function boostTokens(
  ...
  tokens: string[],
  guildId: string,
  opts: { globalName?: string | null; nick?: string | null; bio?: string | null; avatarBase64?: string | null; bannerBase64?: string | null; delayMs?: number }
