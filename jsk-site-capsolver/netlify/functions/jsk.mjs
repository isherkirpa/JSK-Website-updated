...
// ─── CAPSOLVER (primary) + CAPMONSTER (fallback) ─────────────────────────────

async function solveCapsolver(sitekey, pageUrl, apiKey, rqdata = "") {
  try {
    const task = {
  ...
  for (const taskType of taskTypes) {
    ...
    try {
      const createRes = await fetch("https://api.capmonster.cloud/createTask", {
        ...
        body: JSON.stringify({
          clientKey: apiKey,
          task: { type: taskType, websiteURL: pageUrl, websiteKey: sitekey, userAgent: CHROME_UA },
        }),
      });
    ...
    for (let i = 0; i < 30; i++) {
      await sleep(3000);
      try {
        const pollRes = await fetch("https://api.capmonster.cloud/getTaskResult", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ clientKey: apiKey, taskId }),
        });
...
async function solveHcaptcha(sitekey, pageUrl, rqdata = "") {
  if (CAPSOLVER_API_KEY) {
    const token = await solveCapsolver(sitekey, pageUrl, CAPSOLVER_API_KEY, rqdata);
    if (token) return token;
  }
  if (CAPMONSTER_API_KEY) {
    return solveCapmonster(sitekey, pageUrl, CAPMONSTER_API_KEY);
  }
  return null;
}
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    try {
      ...
      if (r.status === 400 && respJson.captcha_key) {
        ...
        const captchaToken = await solveHcaptcha(sitekey, "https://discord.com", rqdata);

        if (!captchaToken) {
          setMsg(
            (CAPSOLVER_API_KEY || CAPMONSTER_API_KEY)
              ? "Captcha required — solver could not solve it. Check balance at capsolver.com"
              : "Captcha required — add CAPSOLVER_API_KEY to Netlify env vars"
          );
          results.push(entry);
...
export const handler = async (event) => {
  ...
  try {
    ...
    // ── SERVER JOINER — proxied to bot server (Netlify timeout = 10s, CapMonster needs 90s) ──
      ...
      // ── SERVER LEAVE — proxied to bot server ─────────────────────────────────
